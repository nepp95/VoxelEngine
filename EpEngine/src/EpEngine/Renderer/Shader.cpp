...
namespace EpEngine {
	static GLenum ShaderTypeFromString(const std::string& type)
	{
		...
		EP_CORE_ASSERT(false, "Unknown shader type!");
		return 0;
	}
	...
	std::unordered_map<GLenum, std::string> Shader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos) {
			...
			EP_CORE_ASSERT(eol != std::string::npos, "Syntax error");
	...
	void Shader::Compile(std::unordered_map<GLenum, std::string> shaderSources)
	{
		...
		EP_CORE_ASSERT(shaderSources.size() <= 2, "We only support 2 shaders for now!");
		...
		for (auto& kv : shaderSources) {
			...
			if (isCompiled == GL_FALSE) {
				...
				EP_CORE_ERROR(infoLog.data());
				EP_CORE_ASSERT(false, "Vertex shader compilation failure!");
		...
		if (!isLinked) {
			...
			EP_CORE_ERROR(infoLog.data());
			EP_CORE_ASSERT(false, "Shader link failure!");
