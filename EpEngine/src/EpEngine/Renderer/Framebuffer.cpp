...
namespace EpEngine
{
	...
	namespace Utils
	{
		...
		static GLenum EngineFBTextureFormatToGL(FramebufferTextureFormat format)
		{
			...
			EP_CORE_ASSERT(false, "Unknown FramebufferTextureFormat!");
			return 0;
		}
	}
	...
	void Framebuffer::Invalidate()
	{
		...
		{
			EP_CORE_ASSERT(m_colorAttachments.size() <= 4, "Too many color attachments!");
		...
		EP_CORE_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete!");
	...
	{
		EP_CORE_ASSERT(attachmentIndex < m_colorAttachments.size(), "Index is out of range!");
	...
	{
		EP_CORE_ASSERT(index < m_colorAttachments.size(), "Index is out of range!");
